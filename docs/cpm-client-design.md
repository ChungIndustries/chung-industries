...
- `cpm.json` at the package root is the manifest and authoring-side source of truth: { name, version, description?, author?, dependencies? }. It ships in the tarball (and so in the bundle, making installed packages self-describing); the registry parses it at publish; the former multipart `meta` field is gone, the tarball is the whole publish request. Future fields (bin aliases, entry overrides) have a natural home here.
- `startup` in `cpm.json` (optional): the path, relative to the package root, of a Lua file to run at computer startup. The registry rejects a publish whose declared startup file is not in the tarball. On install the client writes `/startup/60_cpm_<name>.lua`, which prepends the package path and runs the file; on remove (or when a new version drops the field) the hook is deleted. Hooks run after `50_cpm.lua`, so the shell path is already set. Startup files run sequentially, so a long-running daemon should put itself in the background (`multishell.launch` or `parallel` from its own code); cpm does not wrap it. cpm's own manifest declares one (added 2026-09-01): `startup.lua` registers shell tab-completion for the cpm program, generated by the cli library's `completionFunction` from the same command declarations the program runs. Completion has to be registered before the user types, which is what makes it a startup concern rather than a program one; the completer is keyed by resolved program path without the leading slash (`cpm/bin/cpm.lua`), matching `shell.complete`'s lookup.
...
- The installer is a small, dependency-free, single-file Lua script that: checks `http` is enabled, resolves cpm's pinned dependency closure via `POST /resolve` (cpm is published as a normal package named `cpm`), downloads and installs every package in it (the client requires its dependencies, such as the `cli` library, up front, so a partial bootstrap could not run), then delegates the rest (shims, startup hooks for packages declaring one, `/startup/50_cpm.lua`, shell path, require hook) to the freshly extracted cpm package's own `store.lua` so `cpm` works immediately in the current session without reboot; the freshly written startup hooks are also run once, so boot-only behavior like cpm's tab-completion is live in the install session too. It trusts HTTPS for this first hop instead of verifying digests; the installed client verifies every bundle from then on.
...
- **Publishing tooling for authors**: resolved 2026-09-02 by `apps/cpm-tool`, a Go CLI (`cpm login` / `logout` / `whoami` / `pack` / `publish`) for real computers, with a committed `cpm.json` per package as the manifest source and the registry's OpenAPI spec as the shared type contract. Publish auth shipped earlier (docs/cpm-registry-auth-design.md). Still open there: distribution as standalone binaries with an install script, and a device-flow login to replace pasting a token.
