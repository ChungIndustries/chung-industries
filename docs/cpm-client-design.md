...
3. **Ad-hoc user scripts** using cpm libraries: covered by the require hook (below), no boilerplate. `dofile("/cpm/boot.lua")` remains as a manual fallback for environments where the hook is not active.

**The require hook** (added 2026-08-25, replacing the dofile boilerplate as the primary mechanism): every program environment in CC:Tweaked is created and passed through the global `load`, resolved at call time. Verified against the mc-1.20.x source: the shell's `executeProgram` calls `load(contents, "@/"..path, nil, env)` with the env from `createShellEnv` (which carries `package` from `cc.require.make`); bios `loadfile` calls the global `load`, so `os.run` flows through it too; the `lua` REPL evaluates input via `load(input, name, "t", tEnv)` with its own `cc.require` package; and none of them capture `load` as a local. So `/startup/50_cpm.lua` wraps `_G.load` once per boot: when a chunk is loaded with an env whose own `package.path` (via `rawget`, to skip inheritance through `__index = _G`) lacks the cpm prefix, the prefix is prepended. Result: `require("pkg")` works in any program, from any directory, and in the `lua` REPL. The shim prepend (case 1) is kept as belt and braces. Accepted risk: it is a monkey-patch of a core global; if a future CC version restructures its loaders the hook degrades to a no-op and the shims plus `boot.lua` fallback still work.
...
| Install layout        | Global flat store `/cpm/packages/<name>/`, one version per package; shims in `/cpm/bin` + `/startup/50_cpm.lua` with the global require hook; `dofile("/cpm/boot.lua")` kept as fallback only                  |
...
- **Ad-hoc script ergonomics**: resolved 2026-08-25 by the require hook in section 4.2 (wrapping the global `load` from the startup drop-in). `dofile("/cpm/boot.lua")` survives only as the documented fallback. An upstream CC:Tweaked extra-path setting would still be the cleaner mechanism if one ever appears.
