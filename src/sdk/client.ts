...
import {
  ...
  type PackageRetrieveVersionResponse,
  type PackageResolveResponse,
  type PackageCreateParams,
  type PackageRetrieveVersionParams,
  type PackageResolveParams,
} from './resources/packages/packages';
import { Bootstrap, type BootstrapListInstallResponse } from './resources/bootstrap';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env["CHUNGINDUSTRIES_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["CHUNGINDUSTRIES_LOG"] or 'warn' if it isn't set.
   */
...
export type CpmRegistryOptions = ClientOptions;

/**
 * API Client for interfacing with the CpmRegistry API.
 */
export class CpmRegistry {
  baseURL: string;
  maxRetries: number;
  timeout: number;
  logger: Logger;
   ...
   * API Client for interfacing with the CpmRegistry API.
   *
   * @param {string} [opts.baseURL=process.env["CHUNGINDUSTRIES_BASE_URL"] ?? https://registry.cpm.chungindustries.com] - Override the default base URL for the API.
   ...
   */
  constructor({ baseURL = readEnv('CHUNGINDUSTRIES_BASE_URL'), ...opts }: ClientOptions = {}) {
    const options: ClientOptions = {
      ...opts,
      baseURL: baseURL || 'https://registry.cpm.chungindustries.com',
    };
    const baseURLOverridden = baseURL !== null && baseURL !== undefined && baseURL !== '';
    const defaultBaseURL = 'https://registry.cpm.chungindustries.com';
    this.baseURL = options.baseURL || defaultBaseURL;
    this.timeout = options.timeout ?? CpmRegistry.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CHUNGINDUSTRIES_LOG'), 'process.env["CHUNGINDUSTRIES_LOG"]', this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('CHUNGINDUSTRIES_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  private async resolveAuthOption(
    ...
    if (!token)
      throw new Errors.CpmRegistryError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }
  ...
  static CpmRegistry = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static CpmRegistryError = Errors.CpmRegistryError;
...
CpmRegistry.Packages = Packages;
CpmRegistry.Bootstrap = Bootstrap;

export declare namespace CpmRegistry {
  ...
  export { Bootstrap as Bootstrap, type BootstrapListInstallResponse as BootstrapListInstallResponse };
}
