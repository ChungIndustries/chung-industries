...
- The package index is the registry's `GET /search`: matching and ranking happen there, and the
  site pages through the results (`src/package/queries.ts`). The input responds instantly from
  local state while the URL, and with it the query, follows after a short debounce.
...
Tests cover the pure logic (result paging, bundle parsing, JSend unwrapping): `nx test cpm-web`.
