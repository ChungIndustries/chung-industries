...
/** Suspends until the README arrives; a missing one resolves to null. */
export function ReadmeTab({ version }: { version: PackageVersion }) {
  const { data: readme } = useSuspenseQuery(readmeQueryOptions(version.name, version.version));
  if (!readme) {
...
/** Suspense fallback shaped like a short README. */
export function ReadmeTabSkeleton() {
  return (
